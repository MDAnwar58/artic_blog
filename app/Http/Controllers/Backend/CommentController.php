...
class CommentController extends Controller
{
   ...
   public function destroy($id)
   {
     ...
     if ($comment->child->count() > 0) {
       ...
       return redirect()->route('admin.comment.index')->with('success', 'Comment has been deleted successfully');
     }else{
      $p_id = $comment->p_id;
      $comment->delete();

      return redirect()->route('admin.comment.show', $p_id)->with('success', 'Comment has been deleted successfully');
    }
